...
\begin{figure}
           \centering
           \includegraphics[width=0.45\textwidth]{zentral}
...
\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{dezentral}
...
\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{schreiber}
...
\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{lotterie_ohne}
...
Die Kosten des Hash-verfahrens bietet den weiteren Vorteil, dass eine Manipulation des Ledgers durch den Schreiber nicht rentabel ist.
Die restlichen Teilnehmer können die Änderungen sehr einfach prüfen und notfalls ablehnen, womit der Schreiber
sämtliche investierte Rechenkraft verschwendet hat.
...
\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{halving}
...
Hier kommt  die sogenannte \textit{Blockchain} ins Spiel.
...
Blockes und den Arbeitsnachweis des Schreibers, die sogenannte \textit{Proof of Work}.
Ausgenommen ist hier der erste Block der Blockchain, auch \textit{Genesis-Block} genannt, welcher keine Informationen
...
Der Hash des vorigen Blockes garantiert die Unveränderbarkeit des Blockes, denn um nun einen älteren Block zu manipulieren,
müsste ein Angreifer die Informationen in dem Block so manipulieren, dass der selbe Hash entsteht, also eine Kollision
verursachen, was bei heutigen Hash-Verfahren quasi unmöglich ist.%todo Nachweis
Eine Manipulation eines Blockes würde somit alle nachfolgenden Blöcke unbrauchbar machen.
...
\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{lotterie}
    \caption{Einbinden der Blockchain in Lotterie}
    \label{fig:lotterie}
\end{figure}

Der Gewinner der Lotterie macht nun nicht einfach Einträge in ein Ledger, stattdessen schreibt er einen neuen Block in
...
Verteilte Teilnehmer im Netzwerk starten ihren Versuch in der Lotterie also mit unterschiedlichen Transaktionen.
...
zu sogenannten \textit{Blockkollisionen}.
...
Um nun ein Spalten der Blockchain zu vermeiden und einen allgemeinen Konsens herzustellen muss ein weiteres Konsens-
Verfahren eingeführt werden.\\
Im Falle von Bitcoin heißt dieser Konsens \textit{Nakamoto-Konsens}.
Dieser besagt, dass die Blockchain mit dem höchsten Arbeitsaufwand, \textit{Proof of Work}, akzeptiert wird.
...
Hier kollidieren die Blöcke \textit{B} und \textit{C}, wobei Block \textit{D} Block \textit{C} zuerst bestätigt.
Block \textit{B} wird nun verworfen.
Verworfene Transaktionen gehen allerdings nicht verloren, sie werden in einem späteren Block neu aufgenommen und solange
mit neuen Transaktionen im sogenannten \textit{Transaktionspool} gehalten, den alle Nutzer bei sich halten.

\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{kollision}
    \caption{Nagamoto Konsens}
    \label{fig:kollision}
\end{figure}

Es wurde nun ein Protokoll geschaffen, welches ein dezentrales Ledger durch eine Blockchain realisiert, die jeder Nutzer
der Währung halten und verifizieren kann.
Alle Teilnehmer des Netzwerks sind gleichberechtigt und dürfen somit alle neue Blöcke schreiben, Voraussetzung für eine
Schreibberechtigung ist jedoch das finden einer gültigen Zufallszahl, \textit{Nonce}, welche sobald sie gefunden wurde
als Arbeitsnachweis, \textit{Proof of Work}, von allen Teilnehmer akzeptiert wird.
Die Kosten der Berechnung machen Manipulationsversuche unwirtschaftlich, da diese leicht erkannt und abgelehnt werden
können.
Für den Aufwand des Hashens wird der Schreiber mit der \textit{Coinbase-Transaktion} und Transaktionsgebühren belohnt.
Die \textit{Coinbase-Transaktion} reguliert ebenfalls die Gesamtmenge der Währungseinheiten, da sie nach einer bestimmten
Anzahl an geschriebenen Blöcken im Wert halbiert wird.
Das verhinder eine Inflation der Währung.
